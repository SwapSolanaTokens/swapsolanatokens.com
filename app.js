    ...
    //
    // IMPORTANT LIMITATION (found after the resource-error listener below
    // still missed real failures in production): the widget renders inside
    // a cross-origin iframe (app.debridge.com). Browsers never let error
    // events from resources loaded *inside* a cross-origin iframe's own
    // document reach a listener on our parent `window` — capture phase
    // does not change this, it's a hard browsing-context boundary. So
    // debridgeResourceErrorDetected below can only ever catch failures on
    // resources requested directly in OUR document (e.g. the widget.js
    // <script> tag itself, already separately handled by script.onerror in
    // reloadDebridgeScript). It cannot see the actual chunk-XXXX.js 404s
    // happening inside the iframe, which is the real failure mode. Combined
    // with debridgeLooksHealthy()'s height check — which also passes here,
    // since the iframe's outer shell (logo, feedback button, cookie notice)
    // still renders at full height even when the inner swap form doesn't —
    // a broken mount can look "healthy" to us on the very first attempt and
    // no automatic retry ever fires. That matches what was actually
    // observed: the page loads with the bridge panel visibly incomplete
    // and stays that way until the user manually refreshes a few times.
    // Since we can't inspect or get a reliable "ready" signal from inside
    // the iframe (deBridge's public widget API has no documented
    // ready/rendered event), and since forcing a blind extra remount to
    // "double-check" a working widget turned out to risk breaking it (that
    // was tried and reverted — see the note above initDebridge()), the
    // mitigation for this specific false-healthy case is the persistent
    // "Bridge panel didn't load right? Retry" link under the widget, not
    // automatic detection.
    ...
    // ever notices.
    //
    // This listener only catches resource failures on elements in OUR OWN
    // document (capture phase does not cross a cross-origin iframe
    // boundary — the browser never lets those events reach us). In
    // practice the actual chunk-XXXX.js 404s happen inside the widget's own
    // iframe document and are invisible here; this only helps for the
    // narrower case of a resource we ourselves requested (e.g. widget.js)
    // failing. It's kept as a cheap extra signal, but it is NOT a reliable
    // detector for the real failure — see the note above initDebridge()
    // for how that's actually handled (a persistent manual retry link,
    // since automatic detection can't be trusted without risking breaking
    // an already-working widget).
    ...
    function showDebridgeRetryButton() {
      ...
      if (btn) {
        btn.addEventListener("click", forceDebridgeRemount);
      }
    }

    // Full teardown + fresh mount, restarting the attempt/confirmation
    // counters from zero. Used by both the manual "Retry bridge" button
    // above (shown only after every automatic attempt has failed) and by
    // the small persistent "didn't load right?" link shown under the
    // widget at all times — since a broken mount can report a false
    // "healthy" to our own checks (see the note above mountDebridgeWidget),
    // that link is a one-click fallback for whenever the automatic
    // confirmation passes in initDebridge() still miss a broken render.
    function forceDebridgeRemount() {
      const container = document.getElementById("debridge-widget");
      if (container) container.innerHTML = "";
      initDebridge(1, 0);
    }

    function initDebridgeManualRefreshLink() {
      const link = document.getElementById("debridge-manual-refresh-link");
      if (link) {
        link.addEventListener("click", forceDebridgeRemount);
      }
    }
    ...
    async function mountDebridgeWidget() {
      ...
      // Auxiliary signal only, not something we gate the retry logic on:
      // deBridge's widget object communicates with its iframe over
      // postMessage, so these DO fire across the origin boundary (unlike
      // the plain resource "error" event above). None of them is
      // documented as a definitive "fully rendered" signal, but seeing any
      // of them at least confirms the widget is alive and talking to us.
      // Wrapped in try/catch since `widget.on` may not exist on older
      // widget.js builds.
      try {
        ["needConnect", "inputChainChanged", "outputChainChanged"].forEach((evt) => {
          widget.on(evt, () => {
            console.info(`deBridge widget event: ${evt}`);
          });
        });
      } catch (evtErr) {
        console.warn("Could not attach deBridge widget event listeners:", evtErr);
      }
    ...
    // IMPORTANT: this function must never tear down a mount that already
    // looks fine. An earlier version of this code required 2 consecutive
    // "healthy-looking" mounts before trusting the result, forcing a
    // second silent remount ~3s after a successful one — but since
    // deBridge's CDN is itself flaky (roughly half of mounts fail per
    // manual testing), that extra remount had a real chance of breaking a
    // widget that had already loaded correctly, which looked to the user
    // like "it loaded fine, then refreshed itself and broke". That was
    // strictly worse than doing nothing, so it's gone. We only ever retry
    // in response to an actual detected failure (mountDebridgeWidget()
    // throwing); a non-throwing mount is trusted and left alone.
    //
    // This still can't catch the false-"healthy" case (broken content,
    // but the height check and our own-document error listener both pass —
    // see the long comment above mountDebridgeWidget()). For that case the
    // persistent "Bridge panel didn't load right? Retry" link under the
    // widget (see forceDebridgeRemount()) is the safety net — a user-driven
    // fix instead of an automatic one that risks breaking working state.
