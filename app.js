    ...
    function formatPrice(p) {
      // Prices come from a third-party API and end up inside innerHTML in
      // renderTicker(). Only real, finite numbers are allowed through, so
      // nothing but digits, "$" and "." can ever reach the DOM from here.
    ...
    // renderChart() can originate in Jupiter's widget (onFormUpdate), i.e.
    // third-party data, so it is validated before being interpolated into
    // the Birdeye iframe URL. frame-src in the CSP already pins the host;
    // this keeps the path segment well-formed too (defense in depth).
