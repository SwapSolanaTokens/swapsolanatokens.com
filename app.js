    ...
    const DEBRIDGE_MAX_RETRIES = 4;
    const DEBRIDGE_RETRY_DELAY_MS = 1500; // multiplied by attempt number
    const DEBRIDGE_HEALTH_CHECK_DELAY_MS = 3000; // deBridge normally finishes rendering in 1-2s when healthy
    ...
    // or partial container). This checks for an actual iframe after giving
    // the widget its normal render window, so that case is caught too.
    function debridgeLooksHealthy() {
      const container = document.getElementById("debridge-widget");
      if (!container) return false;
      const iframe = container.querySelector("iframe");
      return !!iframe && container.children.length > 0;
    }
    ...
    async function mountDebridgeWidget() {
      ...
      // initDebridge()'s existing retry/reload logic kicks in.
      const healthy = await waitAndCheckHealth(DEBRIDGE_HEALTH_CHECK_DELAY_MS);
      if (!healthy) {
        throw new Error("deBridge widget mounted incompletely (no iframe found after render window)");
      }
    }
    ...
    async function initDebridge(attempt = 1) {
      try {
        ...
        console.error(`deBridge widget init failed (attempt ${attempt}/${DEBRIDGE_MAX_RETRIES}):`, err);
        if (attempt >= DEBRIDGE_MAX_RETRIES) {
          console.error("deBridge widget: giving up after max retries. The bridge panel will stay blank until the next page load.");
          return;
        }
        await wait(DEBRIDGE_RETRY_DELAY_MS * attempt);
        try {
