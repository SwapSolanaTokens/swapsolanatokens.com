...
- Content-Security-Policy is enforced via `<meta>` in `index.html`.
- No inline event handlers reaching outside the sandboxed chart iframe.
- No third-party analytics or trackers.
- Full audit history (SSL Labs, Pentest-Tools, VirusTotal) linked from the site's Security Audits section.
